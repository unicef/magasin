...
In this getting started you will install magasin within your local machine for testing purposes, then you will perform an [end-to-end data analysis](tutorial-overview.qmd) that includes: [exploratory analysis](exploratory-analysis.qmd) of a data source, [create a pipeline](./automate-data-ingestion.qmd) to automate data ingestion and [create a dashboard](create-a-dashboard.qmd) to present your findings.

Let's start by setting getting the pre-requisites:

## Pre-requisite: a kubernetes cluster

Prior to installing magasin,  you need to have a kubernetes cluster. But don't worry, you can setup one in your local machine very easily. In layman terms, Kubernetes is just a technology that enables managing cloud ready applications, such as magasin. 

For this getting started, we are going to setup a kubernetes cluster through [Docker Desktop](https://docs.docker.com/desktop/), an application that can be installed in any computer. However, if you already have a cluster you can go directly to the [install magasin section](#install-magasin). 
...
:::{.callout-warning}
It is highly recommended to [take a look at the installer script](https://unicef.github.io/magasin/install-magasin.sh) before running it as it will install several components on your system.
...
* **[Start using magasin](../get-started/tutorial-overview.qmd)**. We have created a tutorial that will drive you through the typical steps for enabling a data informed decission organization using magasin.
