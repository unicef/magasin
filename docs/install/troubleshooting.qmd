...
## During installation an error is displayed while installing one component

While installing it may happen that any of the components is not properly installed. You may see a message like the one below:

![Error during install](../images/install/install-error-install-chart.png)

Once the setup has finished you can check the status of all components: 
